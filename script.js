    ...
    if (k == quant) {
      if (pontosControle) {
        for (c = 0; c < curvas[k].length; c++) {
          mostrarPonto(curvas[k][c].x, curvas[k][c].y, "rgba(91, 101, 192, 1)");
        }
      }
      if (poligonaisControle) {
        desenharReta(curvas[k], "rgba(91,101,192,1)", 1);
      }
      if (exibirCurva) {
        desenharCurva(curvas[k], "rgba(91,101,192, 1)", 3);
      }
    }
    else {
      if (pontosControle) {
        for (c = 0; c < curvas[k].length; c++) {
          mostrarPonto(curvas[k][c].x, curvas[k][c].y, "rgba(91, 101, 192, 0.25)"); 
        }
      }
      if (poligonaisControle) {
        desenharReta(curvas[k], "rgba(91, 101, 192, 0.25)", 1);
      }
      if (exibirCurva) {
        desenharCurva(curvas[k], "rgba(91, 101, 192, 0.25)", 1);
      }
    }
  }
}
